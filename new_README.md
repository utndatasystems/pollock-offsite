...
- `pollock` is the main source folder for the Pollock benchmark: it contains the files necessary to generate the polluted versions of an input file (`polluters_base.py`, `polluters_stdlib_v1.py`, and `polluters_stdlib_v2.py`) as well as the files with the metrics to evaluate results of data loading. The test suite now lives under `tests/` so coverage can grow as a first-class feature.
