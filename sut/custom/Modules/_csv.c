...
typedef struct {
    ...
    PyObject *malformed_rows;   /* list of (line_num, raw_line, reason) for bad rows */
} ReaderObj;
...
Reader_iternext(ReaderObj *self)
{
    ...
    while (1) {
        ...
        if (parse_ok && parse_process_char(self, module_state, EOL) < 0)
            parse_ok = 0;

        if (!parse_ok) {
            /* Collect the exception message, then recover and continue. */
            PyObject *exc = PyErr_GetRaisedException();
            PyObject *reason = exc ? PyObject_Str(exc) :
                                     PyUnicode_FromString("parse error");
            Py_XDECREF(exc);
            PyObject *entry = PyTuple_Pack(3,
                PyLong_FromUnsignedLong(self->line_num),
                lineobj,
                reason ? reason : Py_None);
            Py_XDECREF(reason);
            Py_DECREF(lineobj);
            if (entry == NULL)
                return NULL;
            int rc = PyList_Append(self->malformed_rows, entry);
            Py_DECREF(entry);
            if (rc < 0)
                return NULL;
            if (parse_reset(self) < 0)
...
static struct PyMemberDef Reader_memberlist[] = {
    ...
    { "malformed_rows", T_OBJECT, R_OFF(malformed_rows), READONLY },
    { NULL }
};
...
csv_reader(PyObject *module, PyObject *args, PyObject *keyword_args)
{
    ...
    self->field_size = 0;
    self->line_num = 0;
    self->malformed_rows = PyList_New(0);
    if (self->malformed_rows == NULL) {
        Py_DECREF(self);
        return NULL;
    }
